...
class AdraApiTcp : public AdraApiBase {
 public:
   ...
   * @param   int     udp_port  UDP port of EtherNet module. The default value is 5001.
   ...
   */
  AdraApiTcp(char *ip, int port = 6001, int bus_type = BUS_TYPE::UTRC, int is_reset = 1, int udp_port = 5001,
             int baud = 0xFFFFFFFF) {
    if (bus_type == BUS_TYPE::UTRC) {
      if (is_reset) reset_net_rs485(ip, port, udp_port);
      ...
      if (is_reset) reset_net_can(ip, port, udp_port);
 ...
 private:
  ...
  void reset_net_can(char ip[], int tcp_port, int udp_port) {
    ...
    printf("[Adra TCP] Reset Net Step1: connect to tcp\n");
    ...
    printf("[Adra TCP] Reset Net Step2: connect to udp\n");
