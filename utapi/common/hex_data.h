...
class HexData {
 public:
  ...
  static int32_t hex_to_int32(uint8_t *a) { return ((a[3] << 24) + (a[2] << 16) + (a[1] << 8) + a[0]); }
  ...
  static void hex_to_nfp32_big(uint8_t *datahex, float *dataf, int n) {
    for (int i = 0; i < n; ++i) dataf[i] = hex_to_fp32_big(&datahex[i * 4]);
  }
  ...
  static void hex_to_nfp32(uint8_t *datahex, float *dataf, int n) {
    for (int i = 0; i < n; ++i) dataf[i] = hex_to_fp32(&datahex[i * 4]);
  }
